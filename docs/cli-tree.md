...
ChatImg `0.1.8` 使用 ChatStyle 的共享 Click tree runtime，从实际注册的 command surface 生成树。源码模块入口和 console script 都以公开 CLI 名 `chatimg` 作为 canonical root。
...
- `openai` / `crs` 使用 `OPENAI_API_KEY`；Images 为默认，Responses 需显式选择。命名 profile 隔离，且不 fallback 到 OAuth token。
